<?php
    ...
    {
        $sqlMax = "SELECT COALESCE(MAX(id_proveedor),0) AS no_registro FROM emisores_proveedores_cuentas WHERE id_emisor=".$_SESSION['id_emisor'];
        $resMax = mysqli_query($conexion, $sqlMax);
        $ultimo = mysqli_fetch_array($resMax);

        $nuevo_id = $ultimo['no_registro'] + 1;

        $sqlInsert = "INSERT INTO emisores_proveedores_cuentas VALUES(".$nuevo_id.",".$_SESSION['id_emisor'].",".$_POST['id_proveedor'].",'".$_POST['rfc_banco']."','".$_POST['nombre_banco']."','".$_POST['cuenta']."',1)";
